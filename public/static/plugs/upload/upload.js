$(function() {
	layui.use([ 'layer', 'element', 'form', 'upload' ], function() {
		...
		var uploadFile = upload.render({
			elem : '#uploadImg', //绑定元素
			url : 'api/upload', //上传接口
			exts : 'jpg|png|jpeg|heic|pdf',//限定上传类型，配合acceptMime使用
			//accept: images,//指定允许上传时校验的文件类型 images（图片）、file（所有文件）、video（视频）、audio（音频）
			acceptMime : 'images',//只筛选上述类型图片
			// number: '0',//0为不限制上传数量
			// xhr : xhrOnProgress,
			...
			*/
			/*allDone: function(obj){ //当文件全部被提交后，才触发  开启多文件上传时该回调才可触发
				...
				{	如果上传后，出现文件下载框（一般为ie下），那么你需要在服务端对response的headr设置 Content-Type: text/html
  					"code": 0,
  					"msg": "",
 					"data": {
    					"src": "http://google.com"
				...
				*/
				...
				if (res.code == 1) {//此处只用于单文件图片的上传，若为多图片多文件请做其他处理
                    ...
                    $("#imgList").append("<tr>" +
						...
						"<td><button type='button' onclick='deleteImg(this)' class='layui-btn layui-btn-xs layui-btn-danger demo-delete'>删除</button></td>" +
